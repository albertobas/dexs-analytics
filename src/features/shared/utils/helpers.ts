...
import { PoolExtended, PoolsStateData } from 'src/features/shared/pools/core/entities/Pools';
import { TokenExtended, TokensStateData } from 'src/features/shared/tokens/core/entities/Tokens';
...
export const shouldFetch = (stateData: PoolsStateData | TokensStateData | null | undefined, network: string) => {
  // Should fetch data if they have not been fetched already or if they have been fetched more than 15 mins ago
  const time = getTime();
  const data = stateData && stateData[network];
  return !data || data.lastUpdated < time - 900;
};
